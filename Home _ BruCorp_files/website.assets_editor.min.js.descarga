...
var self=this;this._saving=true;this.trigger_up('edition_will_stopped',{noWidgetsStop:true,});const destroy=()=>{self.wysiwyg.destroy();self.trigger_up('edition_was_stopped');self.destroy();};if(!this.wysiwyg.isDirty()){destroy();window.location.reload();return;}
...
function clickOnText(snippet,element,position="bottom"){return{trigger:snippet.id?`#wrapwrap .${snippet.id} ${element}`:snippet,extra_trigger:"body.editor_enable",content:Markup(_t("<b>Click on a text</b> to start editing it.")),position:position,run:"text",consumeEvent:"click",};}
...
/* /saas_website/static/src/js/dns_menu.js defined in bundle 'website.assets_editor' */
$(function(){odoo.define("saas_website.dns_menu",function(require){"use strict";const core=require("web.core");const session=require("web.session");const Widget=require("web.Widget");var DnsConfirmDialog=Widget.extend({xmlDependencies:["/saas_website/static/src/xml/dns_menu.xml"],template:"saas_website.dns_confirm_dialog",});const trial_info=require("saas_trial.trial_info");var DnsMenu=Widget.extend({xmlDependencies:["/saas_website/static/src/xml/dns_menu.xml"],template:"saas_website.dns_menu",events:{"click":"_onClick",},init:function(parent,db_info){this._super(parent);this.db_info=db_info;this.check_confirm_dialog=true;this.db_activated=db_info.status!=="to_activate";},_onClick(e){if(!this.db_activated){core.bus.trigger("db_activation_requested");e.preventDefault();}else if(this.db_info.mode==="trial"&&this.check_confirm_dialog){e.preventDefault();session.rpc("/saas_trial/domain_info",{}).then((r)=>{if(!r.fqdn){$('#dns-confirm-dialog').modal('show');}else{this.check_confirm_dialog=false;e.currentTarget.click();}});}},});var menu=new DnsMenu(this,trial_info);menu.appendTo($(".oe_promote_menu"));var modal=new DnsConfirmDialog(this);modal.appendTo($("body"));});});;

/* /theme_clean/static/src/js/tour.js defined in bundle 'website.assets_editor' */
odoo.define('@theme_clean/js/tour',async function(require){'use strict';let __exports={};const wTourUtils=require('website.tour_utils');const core=require("web.core");const _t=core._t;const snippets=[{id:'s_cover',name:'Cover',},{id:'s_text_image',name:'Text - Image',},{id:'s_title',name:'Title',},{id:'s_features',name:'Features',},{id:'s_carousel',name:'Carousel',},{id:'s_numbers',name:'Numbers',},{id:'s_three_columns',name:'Columns',},{id:'s_call_to_action',name:'Call to Action',},];wTourUtils.registerThemeHomepageTour("clean_tour",[wTourUtils.dragNDrop(snippets[0]),wTourUtils.clickOnText(snippets[0],'h1'),wTourUtils.goBackToBlocks(),wTourUtils.dragNDrop(snippets[1]),wTourUtils.dragNDrop(snippets[2]),wTourUtils.dragNDrop(snippets[3]),wTourUtils.clickOnSnippet(snippets[3]),wTourUtils.changeOption('ContainerWidth','we-button-group.o_we_user_value_widget',_t('width')),wTourUtils.goBackToBlocks(),wTourUtils.dragNDrop(snippets[4]),wTourUtils.dragNDrop(snippets[5]),wTourUtils.dragNDrop(snippets[6]),wTourUtils.dragNDrop(snippets[7]),]);return __exports;});